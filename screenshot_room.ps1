...
$image_tmp="image_tmp/rooms/$($bc_area_id)/$($bc_map_id)/$($side_id)/$($debug_id)"
if (!(Test-Path $image_tmp))
{
	New-Item -ItemType Directory -Force -Path "$image_tmp"
}

$out_path="image_output/rooms/$($bc_area_id)/$($bc_map_id)/$($side_id)"
...
cmd /c "magick" "montage" "$($image_tmp)/*.png" "-tile" "$($i)x$($j)" "-geometry" "+0+0" "+repage" "png:-" "|" "magick" "convert" "-" "-crop" "$($room_width)x$($room_height)+0+0" "png:-" "|" "pngquant" "-" "--speed=1" "--quality=50-100" "--force" "--nofs" ">" "$out_img_path"
